    ...
    O: OutputTrait<OutputSeedIndexer>,
{
    fn take(&self, seed_iter: &mut dyn Iterator<Item = f32>, output: &O) {
        ...
        ResourceGeneration::new(self.ammo, ResourceType::Ammopack, None).take(seed_iter, output);
        ResourceGeneration::new(self.tool, ResourceType::ToolRefillpack, None)
            .take(seed_iter, output);
        IgnoreConsumer::new(
            (self.artifact_count + self.consumable_in_container * 2 + self.consumable_in_worldspawn)
                as usize,
        )
        .take(seed_iter, output);
    }
