...
where
    ...
    fn take(&mut self, seed: f32, output: &mut O) -> bool {
        ...
        if self.picked_zones.len() > 0 && (self.placement_data_index + self.skip_per_item - self.item_loc_id) % self.skip_per_item == 0 {
        ...
        self.item_count == 0 && self.picked_zones.is_empty() && self.placement_data_index % self.skip_per_item == 0
    }
