...
impl CollectableMapper {
    ...
    pub fn load_from_web() -> Option<Self> {
        let link = "https://raw.githubusercontent.com/Tgb03/GTFO_LogReader/master/resources/collectable_maps.ron";
        ...
        match result {
            Ok(res) => Some(res),
            Err(e) => {
                println!("Error trying to load from web collectable maps: {:?}", e);
                None
            }
        }
    }
